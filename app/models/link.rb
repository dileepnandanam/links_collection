class Link < ApplicationRecord
  ...
  def get_youtube_url
    ...
    if host.include? 'youtube.com'
      html = Net::HTTP.get_response(URI.parse(self.url)).response.body
      name, tags = MassEntry.with_info(self.url, html) if [name, tags].any?(&:blank?)
      self.update_attributes(name: name, tags: tags, source_url: 'https://youtube.com/embed/' + url.match(/v=(.*)/)[1])
    end
  end

  def get_xnxx_data
    host = URI.parse(self.url.chomp).host
    ...
    self.update_attributes(name: name, tags: tags, source_url: video_url)
  end
